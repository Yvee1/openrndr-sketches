...
//            val scaling = 1.0
            ...
            fun LineSegment.points(reversed: Boolean = false): List<Pair<Double, Vector2>>{
                return this.contour.movingPositions(stripes*2, (seconds+1.5) / duration % 1, reversed=reversed, offset = if (s.offset > 0.09) 0.3 else s.offset)
            }
            ...
            for (k in -s.yRepeat - 1 until s.yRepeat + 1) {
